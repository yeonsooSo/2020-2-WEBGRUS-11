window.onload = function() {
  buildCalendar();
  document.getElementById("date").value=date.getFullYear()+"년 "+(date.getMonth()+1)+"월 "+date.getDate()+"일 ";
}
...
function dateclick(_date){
  ...
  document.getElementById("date").value=today.getFullYear()+"년 "+(today.getMonth()+1)+"월 "+_date.replace("date","")+"일 ";
}
...
function timeselect(chosentime){
  ...
  document.getElementById("resTime").value = chosentime;
...
function hmpopen(){
  if(!openbool){
    ...
    document.getElementsByClassName("contents_box")[0].style.height = "140vh";
  }else{
    ...
    document.getElementsByClassName("contents_box")[0].style.height = "120vh";
  }
  openbool=!openbool;
}
...
function totalcount(){
  ...
  let hmpdes=[
    ...
    if (sum!=0) {
      ...
      document.getElementById("hmp").value = "성인: "+hmpdes[0]+"명/ 유아: "+hmpdes[1]+"명/ 아동: "+hmpdes[2]+"명";
    }else{
      document.getElementById("hmp").style.color="#CCCCCC";
      document.getElementById("hmp").value = '인원을 선택해 주세요 ';
